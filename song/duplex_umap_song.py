...
from song.util import find_spread_tightness, \
    ...
    train_for_batch_batch_umap, sq_eucl_opt, get_closest_for_inputs, graph_creation
...
class SONG(BaseEstimator):
    ...
    def __init__(self, n_components=2, n_neighbors=1,
                 ...
                 fvc_growth=0.5, chunk_size = 1000, pc_components = 50, non_so_rate = 0, low_memory = False, sampled_batches = False, um_min_dist = 0.001, um_lr = 0.01, um_epochs = 11, vis_neis = 5):
    ...
    def scale_relocate(self, X, scale, loc):
        return X * scale + loc
    ...
    def fit(self, X, reduction = 'PCA', corrected_PC = np.array([])):
        ...
        for i in range(self.max_its):
            ...
            if not i % self.non_so_rate:
                ...
                for chunk in range(n_chunks):
                    ...
                    W_ret, G, E_q_ret = train_for_batch_batch_umap(X_chunk, pdists, i, self.max_its, lrst, self.pow_err, im_neix,
                                                         W[set_ix],
            ...
            if not i%3:
                if reduction == 'PCA' and X[set_ix].shape[1] > 100:
                    W_ = self._get_XPCA(W[set_ix], set_ix).astype(np.float32)
                else:
                    W_ = W[set_ix]
        ...
        if reduction == 'PCA' and X_presented.shape[1] > 100:
            W_ = self._get_XPCA(W[0], 0).astype(np.float32)
        else:
            W_ = W[0]
